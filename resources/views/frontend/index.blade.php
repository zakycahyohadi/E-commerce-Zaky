...
<section id="heroSection" class="container relative max-w-screen-xl pt-10 pb-24">
    <div class="flex flex-wrap items-center justify-between w-full gap-8">
        ...
        <img src="{{ asset('assets/images/Group-163181.png') }}" class="max-w-[584px] max-h-[400px] w-full h-full"
            alt="tickety-assets">
    </div>
</section>
...
<section id="eventSection" class="container relative max-w-screen-xl py-10">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-[30px]">
        ...
        :cover="$event->thumbnail" :title="$event->name" :category="$event->category->name" :date="$event->start_time" :price="$event->start_from"
                                :description="$event->headline" :isPopuler="$event->is_populer" :routes="route('detail', $event->slug)"
        >
        </x-frontend.card-event>
